...
from Alpha_Dataloader_tri import Alpha_Dataset_tri
from Model_GRU_tri import Model_GRU_tri
...
def add_alpha_tri_score(df, mzml, df_prosit):

    df = df.merge(df_prosit, on='pr_id', how='left')
    df = preprocess_info_df(df)

    # prepare xic params
    df = prepare_xic_params(df, mzml)

    # construct feature[spectrum_pred, spectrum_m, pcc]
    X = construct_features(df, mzml)
    X_pr_charge = df['pr_charge'].to_numpy()
    X_seq_len = df['seq_len'].to_numpy()
    y = 1 - df['decoy'].to_numpy()

    # learn from feature
    model = Model_GRU_tri().to(predifine.device)
    loss_fn = torch.nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(model.parameters())

    train_dataset = Alpha_Dataset_tri(X, X_pr_charge, X_seq_len, y, 'training alpha-tri')
    eval_dataset = Alpha_Dataset_tri(X, X_pr_charge, X_seq_len, y, 'scoring alpha-tri')

    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=128,
                                               num_workers=2, shuffle=True, collate_fn=my_collate_tri)
    eval_loader = torch.utils.data.DataLoader(eval_dataset, batch_size=1024,
                                              num_workers=2, shuffle=False, collate_fn=my_collate_tri)

    train_one_epoch(train_loader, model, optimizer, loss_fn)
    print('Alpha-Tri trian finieshed.')

    # append score
    df['score_alpha_tri'] = eval_one_epoch(eval_loader, model)
    median_target = df[df['decoy'] == 0]['score_alpha_tri'].median()
    median_decoy = df[df['decoy'] == 1]['score_alpha_tri'].median()
    print(f'alpha_tri score median for target: {median_target}, '
          f'alpha_tri score median for decoy: {median_decoy}')

    return df

def my_collate_tri(items):
